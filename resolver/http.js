 ...
 * connect to the same URL. This is the only transport the resolver ships.
 *
...
// Per-skill files live in public/r/<name>.json (generated by build-bundle.js).
// We serve GET /skills/<name>.json straight from these — one file per skill — so
// a client can pull exactly the resolved skills without downloading the whole
// bundle. Indexed (name → raw JSON string) once at boot; if public/r is missing,
// the route 404s until `npm run build` has run in scripts/.
const BUILD_DIR = fileURLToPath(new URL("../public/r/", import.meta.url));
let SKILL_JSON_BY_NAME = new Map();
try {
  SKILL_JSON_BY_NAME = new Map(
    readdirSync(BUILD_DIR)
      .filter((f) => f.endsWith(".json"))
      .map((f) => [
        f.replace(/\.json$/, ""),
        readFileSync(BUILD_DIR + f, "utf8"),
      ]),
  );
} catch {
  console.warn(
    "public/r not found — /skills/* will 404 until you run `npm run build` in scripts/",
  );
}
...
if (AUTH_TOKEN) {
  ...
  app.use(SKILLS_ROUTE, requireAuth);
}
...
const LANDING_PAGE = [
  ...
  "</ul>",
  `<p>Skill catalog: <a href="${BUNDLE_ROUTE}"><code>${BUNDLE_ROUTE}</code></a> · one skill: <code>${SKILLS_ROUTE}/&lt;name&gt;.json</code></p>`,
...
// One skill at a time, keyed by name: GET /skills/<name>.json. Served straight
// from public/r/<name>.json so a client pulls exactly the resolved skills without
// downloading the whole bundle. The `.json` suffix is optional (both
// /skills/nextjs and /skills/nextjs.json work).
app.get(`${SKILLS_ROUTE}/:name`, (req, res) => {
  const name = req.params.name.replace(/\.json$/, "");
  const skillJson = SKILL_JSON_BY_NAME.get(name);
  if (!skillJson) {
    return res.status(404).json({ error: `unknown skill: ${name}` });
  }
  res.type("application/json").send(skillJson);
});
