...
4. **`scripts/fetch-skills.js`** — takes a registry **base URL** and a list of matched skill paths (`skills/<category>/<name>`), fetches `GET <url>/bundle.json` (sending `Authorization: Bearer $MCP_AUTH_TOKEN` when that env var is set), and writes each matched skill's `raw` into `.claude/skills/<name>/SKILL.md`, skipping any skill that already exists locally (never overwrites — see Idempotency below). No git clone; a Node ≥18 global `fetch` is the only requirement.
