...
1. **The registry content** — `skills/frontend/`, `skills/backend/`, `skills/database/` directories, each holding `<skill-name>/SKILL.md` (plus `skills/init-project/` for the trigger skill itself). Each `SKILL.md` starts with required YAML frontmatter (`name`, `category`, `match` keyword array, `requires`, `version`) followed by the markdown body (company conventions/patterns actually pulled into consuming projects).
2. **`index.json`** — generated (never hand-edited) by `scripts/generate-index.js`, which scans the category directories under `skills/` for `SKILL.md` frontmatter and flattens it into `{ category: { name: { match, requires, version, path } } }` (where `path` is `skills/<category>/<name>`). This is the single source of truth the resolver reads; `match` keywords live only in the `SKILL.md` frontmatter. Scanning + gray-matter parsing + validation live in `scripts/lib/registry.js` (`collectSkills()`), shared with the bundle build.
   - **`bundle.json`** — also generated (by `scripts/build-bundle.js`), a self-contained catalog that carries each skill's **full markdown body** alongside its metadata (`{ schemaVersion, generatedAt, count, skills: [{ category, name, path, match, requires, version, body }] }`, ordered by category then name). Unlike `index.json` (metadata only, for keyword matching), `bundle.json` lets a client present the whole catalog and let the user pick skills to write into `.claude/skills/` **without cloning the registry**. The MCP tool does not read it; the HTTP entry point (`http.js`) serves it verbatim at `GET /bundle.json` (read once at boot; gated by `MCP_AUTH_TOKEN` when set, since it carries full skill bodies). Regenerate it in the same step as `index.json` (`cd scripts && npm run build`) to avoid drift.
